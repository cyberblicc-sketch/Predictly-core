...
import { formatUSD, formatPct, formatCompact, cn } from '@/lib/utils'
      ...
      <div className="p-4 pb-3">
        <div className="flex gap-3 items-start">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-sm font-semibold leading-snug line-clamp-2 group-hover:text-fg">
              {market.question}
            </h3>
          </div>
        </div>
...
function BinaryOutcomes({
  yesPrice, yesDelta, history,
}: {
  yesPrice: number; yesDelta: number; history: { t: number; price: number }[]
}) {
  ...
  return (
    <div className="space-y-3">
      {/* Big probability number + sparkline */}
      <div className="flex items-end justify-between gap-2">
        <div>
          ...
          <div className={cn(
            ...
            {deltaPos ? '+' : ''}{(yesDelta * 100).toFixed(1)}% 7d
          </div>
        </div>
...
function MultiOutcomeList({
  ...
  outcomes: { id: string; label: string; price: number; delta7d: number }[]
}) {
  return (
        ...
        return (
          <li key={o.id} className="flex items-center gap-2 text-sm">
            ...
            <span className={cn(
              ...
              {deltaPos ? '▲' : '▼'}{Math.abs(o.delta7d * 100).toFixed(0)}
            </span>
            <span className="tabular-nums font-semibold text-fg w-9 text-right">{pct}%</span>
            <button
